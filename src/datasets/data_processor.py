...
class DataProcessor:
    ...
    def sample_random_latent_queries(
        self,
        ...
        if len(phy_domain[0]) == 2:
            ...
            x_coords = torch.FloatTensor(num_tokens).uniform_(x_min, x_max)
            y_coords = torch.FloatTensor(num_tokens).uniform_(y_min, y_max)
            latent_queries = torch.stack([x_coords, y_coords], dim=1)
            if add_extrema:
                corners = torch.tensor([[x_min, y_min], [x_min, y_max], [x_max, y_min], [x_max, y_max]])
                latent_queries = torch.cat([latent_queries, corners], dim=0)
            ...
            x_coords = torch.FloatTensor(num_tokens).uniform_(x_min, x_max)
            y_coords = torch.FloatTensor(num_tokens).uniform_(y_min, y_max)
            z_coords = torch.FloatTensor(num_tokens).uniform_(z_min, z_max)
            latent_queries = torch.stack([x_coords, y_coords, z_coords], dim=1)
            if add_extrema:
    ...
    def generate_regular_latent_queries(
        self,
        token_size: Tuple[int, ...],
    ) -> torch.Tensor:
    ...
    def generate_latent_queries(
        self,
        token_size: Tuple[int, ...],
        method: Literal["regular", "random"] = "regular",
    ) -> torch.Tensor:
        """Generate latent query points based on specified method."""
        if method == "regular":
            return self.generate_regular_latent_queries(token_size)
        elif method == "random":
            return self.sample_random_latent_queries(token_size)
        else:
            raise ValueError(f"Unsupported query generation method: {self.dataset_config.latent_query_method}")
