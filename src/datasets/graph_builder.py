...
class GraphBuilder:
    ...
    def __init__(self, neighbor_search_method_encoder: str = "auto",
                 neighbor_search_method_decoder: str = "auto"):
        """
        Initialize graph builder.

        Args:
            neighbor_search_method_encoder: Method for neighbor search used for the encoder graphs
            neighbor_search_method_decoder: Method for neighbor search used for the decoder graphs
        """
        self.nb_search_encoder = NeighborSearch(neighbor_search_method_encoder)
        self.nb_search_decoder = NeighborSearch(neighbor_search_method_decoder)

    def build_graphs_for_split(
        self,
        x_data: torch.Tensor,
        latent_queries: torch.Tensor,
        gno_radius_encoder: float,
        gno_radius_decoder,
        scales: List[float],
    ) -> Tuple[List, List]:
        """
        ...
        Args:
            ...
            gno_radius_encoder: Base radius for neighbor search (encoder)
            gno_radius_decoder: Base radius for neighbor search (decoder)
        ...
        for i, x_sample in enumerate(x_data):
            ...
            encoder_nbrs_sample = []
            for scale in scales:
                scaled_radius = gno_radius_encoder * scale
                with torch.no_grad():
                    nbrs = self.nb_search_encoder(x_coord_scaled, latent_queries, scaled_radius)
            ...
            decoder_nbrs_sample = []
            for scale in scales:
                scaled_radius = gno_radius_decoder * scale
                with torch.no_grad():
                    nbrs = self.nb_search_decoder(latent_queries, x_coord_scaled, scaled_radius)
...
class CachedGraphBuilder(GraphBuilder):
    ...
    def __init__(
        self, neighbor_search_method_encoder: str = "auto", neighbor_search_method_decoder: str = "auto", cache_dir: Optional[str] = None
    ):
        super().__init__(neighbor_search_method_encoder, neighbor_search_method_decoder)
