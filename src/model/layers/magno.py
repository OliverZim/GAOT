...
class MAGNOConfig:
    ...
    neighbor_search_method_encoder: str = "auto"  # Neighbor search method employed in the encoder, support ['open3d', 'torch_cluster', 'grid', 'chunked', 'native', 'adaptive']
    neighbor_search_method_decoder: str = "auto"  # Neighbor search method employed in the encoder, support ['open3d', 'torch_cluster', 'grid', 'chunked', 'native', 'adaptive']

    use_torch_scatter: bool = True  # Use torch_scatter backend
    neighbor_strategy_encoder: str = (
        "radius"  # Neighbor search strategy, support ['radius', 'knn']
    )
    neighbor_strategy_decoder: str = (
        "radius"  # Neighbor search strategy, support ['radius', 'knn']
    )
...
class MAGNOEncoder(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, config: MAGNOConfig):
        ...
        self.nb_search = NeighborSearch(method=config.neighbor_search_method_encoder)
    ...
    def _compute_neighbors(
        ...
        if mode == "fx":
            ...
            for scale in self.scales:
                scaled_radius = self.config.radius_encoder * scale
...
class MAGNODecoder(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, config: MAGNOConfig):
        ...
        # --- Initialize neighbor search ---
        self.nb_search = NeighborSearch(method=config.neighbor_search_method_decoder)
        self.neighbor_cache = {}
    ...
    def _compute_neighbors(
        ...
        if mode == "fx":
            # Fixed coordinates
            for scale in self.scales:
                scaled_radius = self.config.radius_decoder * scale
                neighbors = self.nb_search(
